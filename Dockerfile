...
#RUN /venv/bin/python3.8 -m pip install --no-cache-dir torch==1.10.1+cu111 -f https://download.pytorch.org/whl/cu111/torch_stable.html
...
#COPY --chown=user:user ./pathology-whole-slide-data-main /tmp/pathology-whole-slide-data
#RUN cd /tmp/pathology-whole-slide-data && pip install --no-cache-dir -e .
...
#RUN cd /mmdetection && pip install --no-cache-dir -e .
# Install MMDetection
...
#COPY --chown=user:user model/Dino3.pth /opt/ml/model1/
